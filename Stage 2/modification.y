...
stmt : _READ '(' _ID ')' { $$ = makeStmtNode(READ, $3, NULL, "READ");}
    | _WRITE '(' expr ')' { $$ = makeStmtNode(WRITE, $3, NULL, "WRITE");}
...
expr : expr _PLUS expr		{ $$ = makeExprNode(PLUS, '+',$1, $3, "+"); }
	...
	| _MINUS expr %prec _UMINUS { $$ = makeExprNode(UMINUS, '-', $2, NULL, "UMINUS"); }
	| _PLUS expr  %prec _UPLUS  { $$ = makeExprNode(UPLUS, '+', $2, NULL, "UPLUS"); }
	| '(' expr ')' 	{ $$ = $2; }
	| _NUM		{ $$ = $1; }
	| _ID		{ $$ = $1; }
%%
